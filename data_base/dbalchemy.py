...
class DBManager(metaclass=SingletonDB):
    ...
    def _add_orders(self, quantity, product_id, user_id, ):
        """ Создание заказа, добавление товара в заказ """
    ...
    def select_all_product_id(self):
        """ Возвращает все id товара в заказе в виде списка"""
        result = self._session.query(Order.product_id).all()
        self.close()
        result_list = [itm[0] for itm in result]
        return result_list

    def select_order_quantity(self, product_id):
        """ Возвращает количество товара product_id в заказе """
    ...
    def select_single_product_quantity(self, product_id):
        """ Возвращает количество товара product_id на складе """
    ...
    def update_product_value(self, product_id, name, value):
        """ Обновляет количество товара product_id на складе """
        self._session.query(Products).filter_by(id=product_id).update({name: value})
        self._session.commit()
        self.close()

    def update_order_value(self, product_id, name, value):
        """ Обновляет данные указанной позиции заказа в соответствии с номером товара """
